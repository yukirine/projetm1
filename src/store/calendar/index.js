...
export const addEvent = createAsyncThunk('appCalendar/addEvent', async (event, { dispatch }) => {
  const response = await axios.post('/calendar/add-event', {
    data: {
      event
    }
  })
  await dispatch(fetchEvents(['Personnel', 'Business', 'RDV', 'Médicaments', 'ETC']))
...
export const updateEvent = createAsyncThunk('appCalendar/updateEvent', async (event, { dispatch }) => {
  const response = await axios.post('/calendar/update-event', {
    data: {
      event
    }
  })
...
export const deleteEvent = createAsyncThunk('appCalendar/deleteEvent', async (id, { dispatch }) => {
  const response = await axios.delete('/calendar/remove-event', {
    params: { id }
  })
  await dispatch(fetchEvents(['Personnel', 'Business', 'RDV', 'Médicaments', 'ETC']))
...
export const appCalendarSlice = createSlice({
  ...
  initialState: {
    events: [],
    selectedEvent: null,
    selectedCalendars: ['Personnel', 'Business', 'RDV', 'Médicaments', 'ETC']
  },
  reducers: {
    ...
    handleAllCalendars: (state, action) => {
      ...
      if (value === true) {
        state.selectedCalendars = ['Personnel', 'Business', 'RDV', 'Médicaments', 'ETC']
      } else {
